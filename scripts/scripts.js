...
/*Query.extend( jQuery.easing,{
	...
	hnlInertial: function(x, t, b, c, d){
		var ts = (t /= d) * t, tc = ts * t;
		return b + c * (-1 * ts * ts + 4 * tc + -6 * ts + 4 * t);
	}
	hnlInertial: function(p){
...
(function(carousel, $, undefined){
	...
	carousel.isNeeded = function($carousel){
		...
		if(Math.ceil(data.itemcount * data.itemwidth) <= Math.ceil(data.slidewidth)){
	...
	carousel.onUpdate = function($carousel, now){
		var data = $carousel.data('carousel');
		
		/*if($carousel.find('.tester').length){
			//var newIndex =  data.index + 1 >= data.$items.length ? 0 : data.index + 1;
			setTimeout(function(){
				var middle = $carousel.offset().left + data.$slide.width() / 2;
				data.$items.each(function(e){
					var position = $(this).offset().left + ($(this).width() / 2);
					var scale = 1 - ((position >= middle ? position - middle : middle - position) / 1000);
					var ratio = 100 - ($(this).find('.callout').outerWidth() / data.itemwidth * 100);
					var left = position >= middle ? -(8 * (1-scale)) * ratio : (8 * (1-scale)) * ratio;
					
					
					
					var z = Math.round(scale / ((data.$items.length/2) / 10));

					$(this).find('.callout').css({
		...
		if(obj.duration > 0){
			...
			data.$slide.stop().animate({
				...
				step: function(now,fx){
					...
					carousel.onUpdate($carousel, fx.now);
			...
			carousel.onUpdate($carousel, obj.position);
					...
					if(!data.disabled){
						...
						$(document).on(move, carouselMove).one(end, function(e){
							...
							if(!scrollable && distanceX){
								...
								acceleration = Math.abs(distanceX / (time.end - time.start)) * 5;
...
addthis_config.pubid = 'ra-572b237edbfb4ac3';

(function(addThis, $, undefined){
	if($('.addthis_toolbox').length && document.location.href.indexOf('file:///') === -1){
		$.getScript('https://s7.addthis.com/js/300/addthis_widget.js#pubid', function(){

		});
	}
}(_.addThis = _.addThis || {}, jQuery));
/***** RESPONSIVE *****/

(function(responsive, $, undefined){
	...
	responsive.breaks = {
		small: function(){
			//console.log('small');
			for(var i=0; i<responsive.methods.small.length; i++){
				responsive.methods.small[i]();
			}
		},
		medium: function(){
			//console.log('medium');
			for(var i=0; i<responsive.methods.medium.length; i++){
				responsive.methods.medium[i]();
			}
		},
		large: function(){
			//console.log('large');
			for(var i=0; i<responsive.methods.large.length; i++){
				responsive.methods.large[i]();
			}
		}
	};
	
	responsive.reset = function(){
		//console.log('reset');
		for(var i=0; i<responsive.methods.reset.length; i++){
			responsive.methods.reset[i]();
		}
	};
	...
	responsive.resize = function(){
		setTimeout(function(){				
			...
			if(breakpoint !== 'false' && !$('html').hasClass(breakpoint)){
				for(var key in responsive.breaks){
					$('html').not('.' + breakpoint).removeClass(key);
				}
				responsive.reset();
				$('html').addClass(breakpoint);
				responsive.breakpoint = breakpoint;
				responsive.breaks[breakpoint]();
			}
			for(var i=0; i<responsive.methods.resize.length; i++){
				responsive.methods.resize[i]();
			}
		},0);
	};
	
	responsive.init = function(){
		responsive.enabled = $('head').css('font-family') !== 'false';
		responsive.resize();
		$(window).on('resize orientationchange', responsive.resize);
	};
	
}(_.responsive = _.responsive || {}, jQuery));
/***** FORM ELEMENTS *****/
...
(function(filter, $, undefined){
	...
	_.filter.init = function(){
		
		var filterLink = $('.filter-link'),
			categoryFilterLink = $('.filter-link.category'),
			countryFilterLink = $('.filter-link.country'),
			countryArr = [],
			categoryArr = [];
		...
		filterLink.on('click', function() {
			...
			if($(this).hasClass('all')){
				categoryFilterLink.removeClass('active');
			...
			if($(this).hasClass('category')) {
				categoryFilterLink.removeClass('active');
				$('.filter-link.all').removeClass('active');
				$(this).addClass('active');
			...
			//Check if it's a country button
			if($(this).hasClass('country')) {
				$(this).toggleClass('active');

				var country = $(this).attr('data-filter-country');
				var foundCountry = $.inArray(country, countryArr);

				if (foundCountry >= 0) {
					countryArr.splice(foundCountry, 1);
				} else {
					countryArr.push(country);
				}
			}

			var activeFilterLink = $('.filter-link.active'),
				block = $('.project-block');

			//Hide all the blocks and the check each projects
			block.removeClass('is-animated')
				.fadeOut()
				.promise()
				.done(function() {
					block.each(function() {
						var blockCategory = $(this).attr('data-category'),
							blockCountry = $(this).attr('data-country');

							//if nothing selected show all
							if(categoryArr.length === 0 && countryArr.length === 0) {
								$(this).addClass('is-animated').fadeIn();
							}
							//else if only the country is selected show the selected country blocks
							else if(categoryArr.length === 0 && $.inArray(blockCountry, countryArr) !== -1) {
								$(this).addClass('is-animated').fadeIn();
							}
							//else if only the category is selected show the selected category blocks
							else if(blockCategory === categoryArr[0] && countryArr.length === 0) {
								$(this).addClass('is-animated').fadeIn();
							}
							//if the category selected & country selected are matches then show the project
							else if(blockCategory === categoryArr[0] && $.inArray(blockCountry, countryArr) !== -1) {
								$(this).addClass('is-animated').fadeIn();
							}
					});
				});
...
(function(cookie, $, undefined){
    ...
    cookie.init = function(overrides){

		var $cookie = $('.cookie');
		
		if(document.location.protocol.indexOf('file:') !== 0 && $cookie.length && !Cookies.get('acceptCookies')){
			
			$('.cookie-accept').on('click', function(e){
				$cookie.animate({
					marginBottom: -$cookie.outerHeight(),
					opacity: 0
				},300,function(){
					$('.cookie').remove();
				});

				if($(e.target).is('.cookie-accept')){
...
(function(gMap, $, undefined){
	
	_.gmap.init = function(){
		...
		/*if(typeof(mapDataEuro) !== 'undefined'){
			var mapEuro = new google.maps.Map(document.getElementById('mapEuro'), {
			  zoom: mapDataEuro.zoom,
			  //zoomControl: false,
			  //scaleControl: false,
			  scrollwheel: false,				
			  center: mapDataEuro,
			  styles: [
				{elementType: 'geometry', stylers: [{color: '#004771'}]},
			...
			var markerEuro = new google.maps.Marker({
			  position: mapDataEuro,
...
(function(lightbox, $, undefined){
	...
	lightbox.close = function(){
		...
		$('.lightbox-content').stop().fadeTo(settings.closeSpeed, 0, function(){
			$('.lightbox-bg').stop().fadeTo(settings.fadeSpeed, 0, function(){
				...
				$('.lightbox').removeClass('lightbox-gallery').hide();
	...
	lightbox.gallery = function(href){
		...
		$('.lightBox[href*=".jpg"], .lightBox[href*=".gif"], .lightBox[href*=".png"], .lightBox[href*=".bmp"], .lightBox[href*=".tiff"], .lightBox[href^="#"]').each(function(){
	...
	lightbox.init = function(overrides){
		...
		if($('.lightbox').length === 0){
			$(settings.wrapingElement).append('<div class="lightbox"><div class="lightbox-content"><span class="lightbox-close">Close</span><span class="lightbox-previous"></span><span class="lightbox-next"></span><div class="lightbox-resize"><div class="lightbox-inner"></div></div></div><div class="lightbox-ghost"></div><div class="lightbox-bg"></div></div>');
		...
		$(document).keydown(function (e){
			...
			if(status.open && key === 27){
				e.preventDefault();
				lightbox.close();
			}else if(status.type === 'image' && galleryItems.length > 1){
				if(key === 37 || key === 80){
					e.preventDefault();
					lightbox.prev();
		...
		$('a.lightBox[href*="#Contact"]').on('click', function(e){
			e.preventDefault();
			var href = $(this)[0].href,
				hash = href.substring(href.indexOf('#'), href.indexOf('?')),
				id = '#' + href.substring(href.indexOf('?') + 1, href.indexOf('=')),
				val = decodeURIComponent(href.substring(href.indexOf('=') + 1)),
				offset = $(hash).offset().top - 20;
			
			$(id).find('[value="' + val + '"]').prop('selected',true);
			$(id).change();
